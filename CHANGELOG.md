...
### Added
- New `animations` setting on every element: animations described as data, so they can be set in a JSON config and are saved with it. Each entry animates a setting (`key`, `from`, `to`, `duration`, `delay`, `loops` - `0` for ever - `yoyo`, and `easing` by name), or with `target: "dataItem"` a value of the element's data item, e.g. a map point's `positionOnLine` to fly it along its line. A saved chart keeps the configured values, not ones caught mid-animation.
- `ChartSerializer`: an animation started in code that loops forever (`loops: Infinity`) is saved as an `animations` entry. Set `runningAnimations: false` to turn serializing looped animations off.
- `ease`: easing functions carry their name (`easingInfo()`), including ones made with `out()`, `inOut()` and `yoyo()`; `byName()` returns one by name.

### Changed
- `MapPointSeries`: an auto-rotating point faces the way it is travelling while its `positionOnLine` animates, so it turns round on the way back.


## [5.20.7] - 2026-09-22

### Added
- `Series`: data items can link somewhere. Set `urlField` to a data field holding a URL, and clicking the item's bullet, column, slice, map polygon or map line opens it; `linkTarget` sets where (`"_self"` by default, like a plain link). Script urls (`javascript:`, `data:`, `vbscript:`) are not opened. A url added to an item later (e.g. `data.setIndex()`) links too.

### Fixed
- `ChartSerializer`: map data rows no longer carry the geodata's own feature properties (`name`, and on some maps `CNTRY`, `TYPE`) - the map copies them onto each row when it loads, so they were written out as if configured. A row left with nothing but its id is dropped.
- `Container`: a `background` with its own `templateField` never read the data item, so a label's background could not be set per data item (e.g. per bullet). It now reads its container's data item.
- `CSVParser`: parsing an empty string looped until it ran out of memory and threw "Invalid array length", instead of returning no rows. An empty response from a server was enough to hit it.
- `JsonParser`: a config where one reference pointed at another further down the list - a legend in an axis header naming a series, as on the candlestick chart demo - failed to parse with "Could not find ref". Such a value is now resolved after the rest of the config.
- `Axis`: something put into an `axisHeader` after the chart was built - a legend, say - did not make the axis leave room for it, so it covered the axis instead of pushing it down. The header's size is no longer held at what it measured while empty, and the axis is brought round when its header's contents change.
...
### Added
- `MapPointSeries`: `positionOnLine`, `autoRotate` and `autoRotateAngle` can now be set in data, so points placed on lines no longer need to be added as data items. `lineId` in data now works without setting `lineIdField`.

### Fixed
- `CategoryAxis`: when a series' data was not in the same order as the axis categories, some of its items were not drawn at all. Most visible on Gantt-like column charts, where several items share a category and the data is ordered by date.
- `ValueAxis`: a logarithmic axis with a wide range labelled its grid with the axis minimum multiplied by ten each step, e.g. 3.01, 30.1, 301. It now uses round powers of ten.
- `ChartSerializer`: `themeTags` given to an element were not serialized, so anything styled through them, like a switch button, lost that styling when the config was parsed again. Tags the library adds itself are still left out.
- Charts with a looping animation, like a pulsing bullet, used ever more memory in Chrome 153 until the tab crashed.
- Underlined text on a layer that was not being redrawn got darker over time.


## [5.20.5] - 2026-09-03

### Fixed
- Column series: switching `colorByDataItem` back off left the columns with the colors it gave them, instead of returning them to the series color.
- Flow charts (`Sankey`/`Chord`/`ArcDiagram`): bullets were misplaced when the series was added to a chart rather than used on its own, e.g. in a `SerialChartContainer`.
- `ValueAxis`: with `syncZeros`, an axis whose data crossed zero was forced to start (or end) at zero when the axis it synced with did not go past zero itself, putting that data outside the axis where it was not drawn at all. Such an axis now keeps its own range - the zero lines cannot line up in that case, but nothing disappears. Give the synced-with axis room past zero (`extraMin`/`extraMax`) to line them up.


## [5.20.4] - 2026-09-02

### Added
- `Entity`: new `once()` and `onceDebounced()` methods - like `on()`/`onDebounced()`, but the callback fires only the first time a setting changes, then removes itself.
- Column series: new `colorByDataItem` setting gives each column its own color from the series' new `colors` (`ColorSet`), instead of the whole series sharing one.

### Fixed
- `SerialChartContainer`: a series' bullets were created but never drawn.
- `ChartSerializer`: serializing a series whose bullet function returns nothing for some data items threw an error.
- `JsonParser`: an adapter whose callback did not survive the trip through JSON broke the chart it was parsed into. Such an adapter is now skipped.
- `JsonParser`: a flow chart's `nodes` are now parsed before its `data`, so links no longer point at nodes that do not exist yet.


## [5.20.3] - 2026-08-25

### Added
- `ChartSerializer`/`JsonParser`: a `Root`'s `interfaceColors` are now serialized and applied on parse.
- `ChartSerializer`/`JsonParser`: a Gantt chart support added.
- `MapLineSeries`: new `pointIds` data field and `pointSeries` setting. A line can now name the points it connects (`{ pointIds: ["JFK", "LAX"] }`) instead of holding their data items. `pointsToConnect` wins if both are set.
- `JsonParser`: new `updateTargets` setting. `"soft"` updates an existing object of the same type instead of replacing it with a new one. Default is `"strict"`, the old behavior.

### Changed
- `ChartSerializer`: only user-set settings are being serialized, leaving defaults out of the resulting JSON.
- `ChartSerializer`: more chart-level elements are serialized: an `XYChart`'s `plotContainer`, anything added to a chart's `children`, a flow's or a map sankey's `nodes`, a word cloud's `shape`, a `ClockHand`'s `hand` and `pin`, and a Gantt's labels, grid, ticks and task parts.
- `ChartSerializer`: objects pointing at one another will now use references instead of copies, e.g. `"@series.get('fill')"`.
- `ChartSerializer`/`JsonParser`: big values are written short and rebuilt on parse: a map's `geoJSON` and a `Root`'s locale by the name of the pack they came from.

### Fixed
- `ChartSerializer`: fixed palette serialization.
- `ChartSerializer`: `type` is stripped from objects an element builds by itself so config does not overwrite it.
- `ChartSerializer`: chart-set values are no longer serialized.
- `ChartSerializer`: a legend or breadcrumb bar added to a container's `children` was written out twice.
- `ChartSerializer`: data a chart builds for itself (e.g. flow's nodes, a word cloud's words) is no longer serialized.
- `ChartSerializer`: templates with states no longer error on serialization.
- `ChartSerializer`/`JsonParser`: data no longer breaks a config (a field called `type`, a string starting with `#` or `@`, and a `null`).
- `ChartSerializer`: a legend fed with data items that are not rows of the series' data (e.g. hierarchy's second level) pointed at the wrong ones and came back empty.
- `Gantt`: collapsing a top-level group was not being written to the data.
- `MapSankeySeries`: some settings did nothing when changed on a chart that was already drawn.
- `XYChart`: series colors are assigned when themes are applied, not only on the first frame.
- `HeatLegend`: `startOpacity` and `endOpacity` are no longer declared required.
- A scrollable `Container` - a `Legend` with a `verticalScrollbar`, say - scrolled to the bottom and then made taller kept its contents pushed up. `scrollToChild()` could leave a similar gap on the last child.
- Background `themeTags` were not being applied properly in some cases.
- Resetting `focusable` to `false` on a `Sprite` was not removing the `focusableElement` private setting.
- The tooltips with `tooltipPosition: "pointer"` would not hide when tap would occur outside target element the toopltip was displayed on.


## [5.20.2] - 2026-08-13

### Added
- New `SerialChartContainer` chart type — a directly-usable `SerialChart` whose `seriesContainer` is a `ZoomableContainer`, for zoomable/pannable series like `ForceDirected`.
- `ArcDiagram`: bullets added to nodes are now positioned on the node circle, using `locationX`/`locationY` as fractions of it. Previously they were created but never placed.
- `Scrollbar`: new `opposite` setting puts a chart's `scrollbarX` below the plot and its `scrollbarY` to the left, instead of above and to the right.
- JSON config now supports a top-level `root` section to apply settings and properties to the `Root` object.
- `Root` now supports `fontFamily`, `fontSize`, and `fontWeight` settings, used as defaults for all text elements unless overridden on an element.
- `ChartSerializer`: new `includeRoot` setting (default `false`) adds a `root` section with the `Root` object's settings and properties to the serialized output.
- `MapChart`: new `boxZoom` setting zooms the map to an area drawn with a pointer. The drawn area is represented by the new `boxZoomSelection` element.
- `MapChart` now zooms in on a double click, and out on a shift + double click. Set the new `doubleClickZoom` setting to `false` to disable it.

### Fixed
- Serializing then parsing a chart no longer breaks on an element's `background`/`icon`: type-less sub-object settings are now merged into the existing element instead of replacing it.
- `XYCursor`: with `clickTolerance` set, a plain click outside the plot area dispatched `"selectended"` with a selection rectangle that was never drawn — the press was clamped onto the plot edge but the release point was not, so the click measured as a drag. It now dispatches `"selectcancelled"`, same as a click within the plot area.
- `WordCloud` ignored its own padding when positioning words: the cloud shrank to fit but stayed in the top left corner, with all the padding pooled at the bottom right.
- Flow charts: bullets added to nodes were drawn underneath the node rectangles. They are now children of the node, drawn above its shape but below its label, with their position relative to the node.
- `FunnelSeries`/`PyramidSeries`: bullet `locationX`/`locationY` positioned against the slice's bounding box, so a bullet could land outside a tapered slice. They now follow the slice's actual sloping edges.
- Percent charts (pie, funnel, pyramid): with more than one series on a chart, bullets of the second and later series were drawn over the first one, ignoring how the chart lays the series out.
- Timeline charts (`SerpentineChart`/`SpiralChart`/`CurveChart`): an axis added after the chart was built was not laid out along the curve until the chart was resized.
...
### Added
- New `StarPattern` and `TrianglePattern` fill patterns added. [More info](https://www.amcharts.com/docs/v5/concepts/colors-gradients-and-patterns/patterns/)
...
- Two new parameterized themes : `Monochrome` (a single-hue lightness ramp with an optional `accent`) and `Adaptive` (generates a full palette from one or two base colors). [More info](https://www.amcharts.com/docs/v5/concepts/themes/)
...
- New properties on Indicator editable settings: `scale`, `maxValue`, `step`. [More info](https://www.amcharts.com/docs/v5/tutorials/creating-custom-indicators-for-a-stock-chart/#Setting_up_the_modal)
...
### Changed
- If an invalid SVG path is passed to Graphics svgPath setting, warning is displayed instead of JS error.
...
- Accessibility: `<div>` element that holds `<canvas>` elements has now `aria-hidden="true"` set.
